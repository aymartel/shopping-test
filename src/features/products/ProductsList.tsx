...
export const ProductsList = (): JSX.Element => {
    ...
    const category = useSelector((state: RootState) => state.products.categoryGet);
    const [loading, setLoading] = useState(false);
    const [page, setPage] = useState(0);
    const maxPerPage = 5;
    const hasMoreData = productsList.length < 100;

    const loadMoreNumbers = () => {

        setLoading(true);
        setTimeout(() => {
            dispatch(fetchProducts({ url, sort: ["name", "ASC"], page, maxPerPage, category }));
            setLoading(false);
        }, 300);
        setPage((page) => page + maxPerPage);
    };
